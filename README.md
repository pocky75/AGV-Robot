...
### 📅 프로젝트 기간
- **개발 기간**: 2025.05.01 ~ 2025.05.29 (4주)
- **대회 참가**: 2025.05.30
...
## 👥 팀원 및 역할 분담
본 프로젝트는 3명의 팀원이 협력하여 하드웨어 개조부터 소프트웨어 제어까지 개발했습니다.

| 이름 | 역할 | 주요 담당 업무 |
|:---:|:---|:---|
| **김선준,김희웅,이용전** | **소프트웨어 및 제어 알고리즘** | PID 라인 트레이싱 로직 구현, 경로 파싱 알고리즘 |
| **이용전,김희웅,김선준** | **하드웨어 및 기구 설계** | 2륜 로봇 4륜 개조 설계, 캐스터 볼 지지대 제작, 모터/센서 배선 및 전원 시스템 구축 |
| **김선준,김희웅,이용전** | **시스템 통합 및 테스트** | IR 센서 임계값 보정, 미션 시나리오별 주행 테스트, VSCode/PlatformIO 개발 환경 세팅 및 디버깅 |
...
| **라인 센서** | IR 센서 어레이 | 바닥 라인 추적 및 편차 감지 |
| **바퀴 구성** | **4륜** (좌우 2개씩) + 캐스터 볼 (뒤쪽) | 4개 바퀴로 하중 분산, 캐스터 볼로 방향 전환 |
| **전원** | Li-Po 배터리 3V | 안정적인 전원 공급 |
...
본 AGV 로봇의 핵심 알고리즘은 크게 **경로 실행, 가변 회전, RFID 인식, PID**로 구성됩니다.
...
void waitCardTag() {
  while (!mfrc522.PICC_IsNewCardPresent()) {
    delay(100); // 카드 감지 시까지 대기
  }
  if (mfrc522.PICC_ReadCardSerial()) {
    buzzer(1000, 100); // 태그 인식 성공 시 피드백
  } else {
    delay(500);
  }
}
...
void moveIrForwardPID(int basePower) { 
  ...
  while (true) {
    ...
    // 2. PID 연산 (Windup 방지 포함)
    integral = constrain(integral + error, -5.0, 5.0);
